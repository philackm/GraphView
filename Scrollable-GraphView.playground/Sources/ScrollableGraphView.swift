...
@objc public class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    public var backgroundFillColor = UIColor.white()
    ...
    public var fillGradientStartColor = UIColor.white()
    /// If fillType is set to .Gradient, then this will be the ending colour for the gradient.
    public var fillGradientEndColor = UIColor.black()
    ...
    public var dataPointFillColor: UIColor = UIColor.black()
    ...
    public var referenceLineColor = UIColor.black()
    ...
    public var referenceLineLabelFont = UIFont.systemFont(ofSize: 8)
    /// The colour of the reference line labels.
    public var referenceLineLabelColor = UIColor.black()
    ...
    public var dataPointLabelColor = UIColor.black()
    /// The colour for the data point labels.
    public var dataPointLabelFont: UIFont? = UIFont.systemFont(ofSize: 10)
    ...
    private func calculateRangeForActivePointsInterval(_ interval: CountableRange<Int>) -> (min: Double, max: Double) {
        
        let dataForActivePoints = data[interval.lowerBound...interval.upperBound]
    ...
    private func calculateRangeForEntireDataset(_ data: [Double]) -> (min: Double, max: Double) {
        let range = calculateRange(self.data)
        return cleanRange(range)
    }
    
    private func calculateRange<T: Collection where T.Iterator.Element == Double>(_ data: T) -> (min: Double, max: Double) {
    ...
    private func cleanRange(_ range: (min: Double, max: Double)) -> (min: Double, max: Double){
    ...
    private func graphWidthForNumberOfDataPoints(_ numberOfPoints: Int) -> CGFloat {
        let width: CGFloat = (CGFloat(numberOfPoints - 1) * dataPointSpacing) + (leftmostPointPadding + rightmostPointPadding)
        return width
    }
    
    private func calculatePosition(_ index: Int, value: Double) -> CGPoint {
    ...
    private func updateLabels(_ deactivatedPoints: [Int], _ activatedPoints: [Int]) {
    ...
    private func setFromClosedRange(_ range: CountableRange<Int>) -> Set<Int> {
        var set = Set<Int>()
  ...
  private func filterPointsForLabels(fromPoints points:[Int]) -> [Int] {
    ...
    private func intervalForActivePoints() -> CountableRange<Int> {
    ...
    private func graphPointForIndex(_ index: Int) -> GraphPoint {
...
private class BarDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    private func createBarPath(_ centre: CGPoint) -> UIBezierPath {
...
private class DataPointDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    private func createCircleDataPoint(_ centre: CGPoint) -> UIBezierPath {
        return UIBezierPath(arcCenter: centre, radius: dataPointSize, startAngle: 0, endAngle: CGFloat(2.0 * M_PI), clockwise: true)
    }
    
    private func createSquareDataPoint(_ centre: CGPoint) -> UIBezierPath {
...
private class ReferenceLineDrawingView : UIView {
    ...
    private func createLabelAtPosition(_ position: CGPoint, withText text: String) -> UILabel {
    ...
    private func createIntermediateReferenceLines(_ rect: CGRect, numberOfIntermediateReferenceLines: Int, forPath path: UIBezierPath) {
    ...
    private func recursiveCreateIntermediateReferenceLines(_ rect: CGRect, width: CGFloat, forPath path: UIBezierPath, remainingPartitions: Int) -> UIBezierPath {
    ...
    private func addLineWithTag(_ tag: String, from: CGPoint, to: CGPoint, inPath path: UIBezierPath) {
    ...
    private func addLineWithGaps(_ from: CGPoint, to: CGPoint, withGaps gaps: [(start: CGFloat, end: CGFloat)], inPath path: UIBezierPath) {
    ...
    private func addLineFrom(_ from: CGPoint, to: CGPoint, inPath path: UIBezierPath) {
        path.move(to: from)
        path.addLine(to: to)
    }
    
    private func boundingSizeForText(_ text: String) -> CGSize {
        return (text as NSString).size(attributes: [NSFontAttributeName:labelFont])
    }
    
    private func calculateYAxisValueForPoint(_ point: CGPoint) -> Double {
